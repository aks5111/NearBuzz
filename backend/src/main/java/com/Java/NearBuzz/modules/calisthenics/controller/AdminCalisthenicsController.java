...
@RestController
@RequestMapping("/api/v1/admin/calisthenics")
public class AdminCalisthenicsController {

    private final CalisthenicsService service;
    private final UserRepository userRepository;

    public AdminCalisthenicsController(CalisthenicsService service, UserRepository userRepository) {
        this.service = service;
        this.userRepository = userRepository;
    }

    @GetMapping
    public ApiResponse<List<CalisthenicsResponse>> list() {
        return ApiResponse.ok("Calisthenics listings fetched", service.listAll());
    }

    @PostMapping
    public ApiResponse<CalisthenicsResponse> create(
            @Valid @RequestBody CalisthenicsRequest request, @AuthenticationPrincipal UserDetails principal) {
    ...
    @PutMapping("/{id}")
    public ApiResponse<CalisthenicsResponse> update(
            @PathVariable Long id, @Valid @RequestBody CalisthenicsRequest request) {
