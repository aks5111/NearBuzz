...
@RestController
@RequestMapping("/api/v1/admin/fitness")
public class AdminFitnessController {

    private final FitnessService service;
    private final UserRepository userRepository;

    public AdminFitnessController(FitnessService service, UserRepository userRepository) {
        this.service = service;
        this.userRepository = userRepository;
    }

    @GetMapping
    public ApiResponse<List<FitnessResponse>> list() {
        return ApiResponse.ok("Fitness listings fetched", service.listAll());
    }

    @PostMapping
    public ApiResponse<FitnessResponse> create(
            @Valid @RequestBody FitnessRequest request, @AuthenticationPrincipal UserDetails principal) {
    ...
    @PutMapping("/{id}")
    public ApiResponse<FitnessResponse> update(@PathVariable Long id, @Valid @RequestBody FitnessRequest request) {
